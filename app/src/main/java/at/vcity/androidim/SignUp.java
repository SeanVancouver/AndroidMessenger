						...
						if (passwordText.getText().toString().equals(passwordAgainText.getText().toString())){
							...
							if (usernameText.length() >= 5 && passwordText.length() >= 5) {
									...
									Thread thread = new Thread(){
										...
										@Override
										public void run() {
											result = imService.signUpUser(usernameText.getText().toString(), //iii105 THIS CALLS SignUpUser
										...
										//	System.out.println(result + "SIGNUP RESULT @@@@@@@@@@@@");
											handler.post(new Runnable(){
												String string = "1";
												public void run() {
													if (result.equals(SERVER_RES_RES_SIGN_UP_SUCCESFULL)) { //iii null object 112
